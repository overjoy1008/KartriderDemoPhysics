...
#define KART_DEMO_SOUND_MENU_BASE 4000U

static unsigned int kart_demo_popup_select_engine_sound(
    HWND window,
    unsigned int current)
{
    static const char *const labels[] = {
        "Classic", "SR", "SR Bike", "Z7", "Z7 Bike", "HT", "HT Bike",
        "JIU", "JIU Bike", "X", "X Bike", "V1", "V1 Bike",
    };
    ...
    AppendMenuA(menu, MF_STRING | MF_DISABLED, 0, "ENGINE SOUND");
    AppendMenuA(menu, MF_SEPARATOR, 0, NULL);
    for (i = 0; i < sizeof(labels) / sizeof(labels[0]); ++i) {
        AppendMenuA(
            menu, MF_STRING | (i == current ? MF_CHECKED : 0),
            KART_DEMO_SOUND_MENU_BASE + i, labels[i]);
    }
    GetWindowRect(window, &window_rect);
    SetForegroundWindow(window);
    command = TrackPopupMenu(
        menu, TPM_RETURNCMD | TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RIGHTBUTTON,
        window_rect.left + 520, window_rect.top + 72, 0, window, NULL);
    DestroyMenu(menu);
    PostMessage(window, WM_NULL, 0, 0);
    if (command >= KART_DEMO_SOUND_MENU_BASE &&
        command < KART_DEMO_SOUND_MENU_BASE + sizeof(labels) / sizeof(labels[0])) {
        return command - KART_DEMO_SOUND_MENU_BASE;
    }
    return current;
}
