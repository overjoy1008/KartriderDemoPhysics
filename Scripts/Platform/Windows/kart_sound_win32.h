...
typedef struct KartDemoSound {
    ...
    int engine_presets[KART_ENGINE_SOUND_PRESET_COUNT][KART_ENGINE_SOUND_SLOT_COUNT];
    unsigned int engine_preset;
    int motor_voice;
    int drift_voice;
...
static void kart_demo_sound_start(HINSTANCE instance, KartDemoSound *sound)
{
    ...
    sound->engine_preset = KART_ENGINE_SOUND_CLASSIC;
    ...
    for (preset = 0; preset < KART_ENGINE_SOUND_PRESET_COUNT; ++preset) {
        for (slot = 0; slot < KART_ENGINE_SOUND_SLOT_COUNT; ++slot) {
            sound->engine_presets[preset][slot] = kart_demo_load_sound_resource(
                instance, sound->audio, IDR_ENGINE_SOUND(preset, slot));
        }
    }
    sound->motor_sound = sound->engine_presets[sound->engine_preset][0];
    sound->drift_sound =
        kart_demo_load_sound_resource(instance, sound->audio, IDR_SOUND_DRIFT);
    sound->booster_sound = sound->engine_presets[sound->engine_preset][1];
    sound->instant_boost_sound = sound->engine_presets[sound->engine_preset][2];
    sound->booster_idle_sound = sound->engine_presets[sound->engine_preset][3];
    sound->count_sound[0] =
...
static void kart_demo_sound_select_engine(
    KartDemoSound *sound,
    unsigned int preset)
{
    if (preset >= KART_ENGINE_SOUND_PRESET_COUNT ||
        preset == sound->engine_preset) return;
    if (sound->audio != NULL) {
        kart_audio_stop_voice(sound->audio, sound->motor_voice);
    ...
    sound->instant_boost_voice = -1;
    sound->booster_idle_voice = -1;
    sound->engine_preset = preset;
    sound->motor_sound = sound->engine_presets[preset][0];
    sound->booster_sound = sound->engine_presets[preset][1];
    sound->instant_boost_sound = sound->engine_presets[preset][2];
    sound->booster_idle_sound = sound->engine_presets[preset][3];
    kart_sound_driver_reset(&sound->driver);
    if (sound->audio != NULL && sound->motor_sound >= 0) {
        sound->motor_voice = kart_audio_play_loop(
            sound->audio, sound->motor_sound, 0.0f, KART_SOUND_MOTOR_BASE);
    }
}
