...
already checks the 26 constants in `Scripts/Runtime/Gameplay/kart_demo_data.c` against the assets.
...
the two extraction commands):
  ...
  Assets/Models/Karts/extracted/<kart>/{model.1s,parameter.xml,*.png}
  Assets/Models/Karts/meshes/<kart>.ktrk
...
Usage:  python DeveloperTools/AssetPipeline/derive_kart_catalog.py
"""
...
# Order follows Scripts/Runtime/Gameplay/kart_demo_data.c so the catalogue lines up with the table
...
def write_doc(karts):
    ...
    w('생성은 `python DeveloperTools/AssetPipeline/derive_kart_catalog.py`, 원본 데이터는')
    ...
    w('회색 카트가 나온다.\n')
    w('`Scripts/Tests/test_kart_model_assets.c`가 26개 모델을 전부 열어 서브메쉬 6개와')
    w('바퀴 4개를 확인하고, 차체 박스에서 `half_width`/`half_length`/')
    w('`model_height`를 다시 계산해 `Scripts/Runtime/Gameplay/kart_demo_data.c`의 상수와 대조한다.')
