...
$nugetConfig = Join-Path $workspace 'DeveloperTools\AssetImporters\rho-safe-index\NuGet.Config'
& dotnet restore (Join-Path $workspace 'DeveloperTools\AssetImporters\rho-safe-index\RhoSafeIndex.csproj') --configfile $nugetConfig
if ($LASTEXITCODE -ne 0) { throw 'RHO indexer restore failed.' }
& dotnet restore (Join-Path $workspace 'DeveloperTools\AssetImporters\track-mesh-exporter\TrackMeshExporter.csproj') --configfile $nugetConfig
if ($LASTEXITCODE -ne 0) { throw 'Track exporter restore failed.' }

& dotnet build (Join-Path $workspace 'DeveloperTools\AssetImporters\rho-safe-index\RhoSafeIndex.csproj') -c Release --no-restore
if ($LASTEXITCODE -ne 0) { throw 'RHO indexer build failed.' }
& dotnet build (Join-Path $workspace 'DeveloperTools\AssetImporters\track-mesh-exporter\TrackMeshExporter.csproj') -c Release --no-restore
